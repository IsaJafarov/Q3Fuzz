...
class HttpClient():
    ...
    def send_ack_frame(self, context: QuicReceiveContext, largest_acknowledged: int) -> None:
        ...
        buf.push_uint_var(500)  # ACK delay
        ...
        self.send_quic_frames_from_builder(builder)
        response_packets = self.read_from_buffer() # drain the buffer
    ...
    def replay_msg(self, h3msg:Packet, respond_with_ack:bool=True) -> str:
        ...
        quic_frames = MSGDissector().dissect_msg(h3msg)
        if QuicAck in quic_frames:
            quic_frames.remove(QuicAck)
