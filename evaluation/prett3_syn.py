...
class HttpClient():
    def __init__(self, quic_conf, hostname) -> None:
        ...
        self.quic_conf.secrets_log_file = open('secrets.keylog', "a")
    ...
    def get_builder(self, epoch: Epoch):
        ...
        builder.start_packet(quic_packet_type, crypto)
    ...
    def send_quic_frames_from_builder(self, builder:QuicPacketBuilder):
        print(">>> send_quic_frames_from_builder: start")
    ...
    ''' for test purposes
    def open_qpack_streamsq(self):
        print(">>> open_qpack_streams: start")
        builder = self.get_builder(Epoch.ONE_RTT)
        self._quic._write_application(builder, network_path, time.process_time())    
        self.send_quic_frames_from_builder(builder)
    '''
...
if __name__ == "__main__":
    ...
    print( QuicFrameType.STREAM_BASE | 2 )
    print( aioquic.buffer.encode_uint_var(StreamType.CONTROL) )
    print( aioquic.buffer.encode_uint_var(StreamType.QPACK_ENCODER) )
    print( aioquic.buffer.encode_uint_var(StreamType.QPACK_DECODER) )
    main(
